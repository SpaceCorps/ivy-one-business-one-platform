...
public class SocialMarketingApp : ViewBase
{
    ...
    {
        return this.UseBlades(() => new SocialMarketingRootBlade(), "Social Marketing", Size.Units(120));
    }
}
...
public class SocialMarketingRootBlade : ViewBase
{
    public override object? Build()
    {
        ...
        var query = context.Campaigns.Where(c => c.Type == "Social Media").AsQueryable();
        ...
        var listItems = campaigns.Select(camp => new ListItem(
            ...
            icon: Icons.Heart,
            badge: camp.Status,
            onClick: _ => blades.Push(this, new SocialCampaignDetailBlade(camp.Id, () => refreshToken.Refresh()), camp.Name)
        ));
        
        var mainContent = BladeHelper.WithHeader(
            Layout.Horizontal()
                .Gap(4)
                .Add(searchQuery.ToSearchInput().Placeholder("Search social campaigns..."))
                .Add(new Button("New Campaign")
                    .Icon(Icons.Plus)
                    .Variant(ButtonVariant.Primary)
                    .HandleClick(_ => isNewCampaignOpen.Set(true))),
            campaigns.Count == 0 
                ? Text.Block("No social media campaigns found!")
                : new List(listItems)
        );

        return isNewCampaignOpen.Value ? new Sheet(
            (Event<Sheet> _) => isNewCampaignOpen.Set(false),
            new SocialCampaignFormSheet(null, () => {
...
public class SocialCampaignDetailBlade(int campaignId, Action? onRefresh = null) : ViewBase
{
    public override object? Build()
    {
        var client = this.UseService<IClientProvider>();
        ...
        return Layout.Vertical()
            ...
            .Add(isEditOpen.Value ? new Sheet(
                ...
                new SocialCampaignFormSheet(campaignId, () => {
...
public class SocialCampaignFormSheet(int? campaignId = null, Action? onClose = null) : ViewBase
{
    public override object? Build()
    {
        var client = this.UseService<IClientProvider>();
            ...
            Layout.Vertical().Gap(4)
                .Add(new Card(
                    Layout.Vertical().Gap(3)
                        ...
                        }).Placeholder("Holiday Campaign 2025"))
